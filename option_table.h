 ...
 *
 * 【文件说明】命令行选项定义表。
 * 定义了 nvidia-installer 的所有命令行选项（100+），包括：
 *   - 选项的长名称和短名称
 *   - 选项的标志（是否需要参数、是否适用于卸载程序等）
 *   - 选项的帮助文本
 *
 * 此文件被 nvidia-installer.c 中的 parse_commandline() 函数使用，
 * 也被 gen-manpage-opts.c 用于自动生成 man 页面。
 *
 * 选项枚举值从 1024 开始，以避免与 ASCII 字符冲突
 * （单字符选项使用其 ASCII 值，如 'v'=118, 'h'=104 等）。
 *
 * option_table.h
 */
...
/* 标志位：标记该选项同时适用于 nvidia-uninstall 程序 */
#define NVGETOPT_OPTION_APPLIES_TO_NVIDIA_UNINSTALL 0x00010000

/* 编译时检查：确保 OPTION_APPLIES_TO_NVIDIA_UNINSTALL 在 nvgetopt 允许的自定义标志范围内 */
...
/*
 * 长选项 ID 枚举。
 * 从 1024 开始以避免与单字符选项的 ASCII 值冲突。
 * 这些值作为 NVGetoptOption 结构的 val 字段，
 * 在 parse_commandline() 的 switch 语句中匹配。
 */
enum {
    XFREE86_PREFIX_OPTION = 1024,         /* --xfree86-prefix（已弃用，同 --x-prefix） */
    OPENGL_PREFIX_OPTION,                 /* --opengl-prefix */
    OPENGL_LIBDIR_OPTION,                /* --opengl-libdir */
    WINE_PREFIX_OPTION,                  /* --wine-prefix */
    WINE_LIBDIR_OPTION,                  /* --wine-libdir */
    KERNEL_INCLUDE_PATH_OPTION,          /* --kernel-include-path（已弃用） */
    KERNEL_INSTALL_PATH_OPTION,          /* --kernel-install-path */
    UNINSTALL_OPTION,                    /* --uninstall */
    SKIP_MODULE_UNLOAD_OPTION,           /* --skip-module-unload */
    SKIP_MODULE_LOAD_OPTION,             /* --skip-module-load */
    PROC_MOUNT_POINT_OPTION,             /* --proc-mount-point */
    USER_INTERFACE_OPTION,               /* --ui */
    LOG_FILE_NAME_OPTION,                /* --log-file-name */
    HELP_ARGS_ONLY_OPTION,               /* --help-args-only（内部使用） */
    TMPDIR_OPTION,                       /* --tmpdir */
    NO_NVIDIA_MODPROBE_OPTION,           /* --no-nvidia-modprobe */
    INSTALLER_PREFIX_OPTION,             /* --installer-prefix（已弃用） */
    FORCE_TLS_OPTION,                    /* --force-tls（已弃用） */
    SANITY_OPTION,                       /* --sanity */
    ADVANCED_OPTIONS_ARGS_ONLY_OPTION,   /* --advanced-options-args-only（内部使用） */
    UTILITY_PREFIX_OPTION,               /* --utility-prefix */
    UTILITY_LIBDIR_OPTION,               /* --utility-libdir */
    XDG_DATA_DIR_OPTION,                 /* --xdg-data-dir */
    ADD_THIS_KERNEL_OPTION,              /* --add-this-kernel */
    RPM_FILE_LIST_OPTION,                /* --rpm-file-list（内部使用） */
    NO_RUNLEVEL_CHECK_OPTION,            /* --no-runlevel-check（已弃用） */
    PRECOMPILED_KERNEL_INTERFACES_PATH_OPTION, /* --precompiled-kernel-interfaces-path */
    PRECOMPILED_KERNEL_INTERFACES_URL_OPTION,  /* --precompiled-kernel-interfaces-url（未使用） */
    NO_ABI_NOTE_OPTION,                  /* --no-abi-note */
    KERNEL_SOURCE_PATH_OPTION,           /* --kernel-source-path */
    NO_RPMS_OPTION,                      /* --no-rpms */
    X_PREFIX_OPTION,                     /* --x-prefix */
    KERNEL_OUTPUT_PATH_OPTION,           /* --kernel-output-path */
    FORCE_TLS_COMPAT32_OPTION,           /* --force-tls-compat32（已弃用） */
    COMPAT32_CHROOT_OPTION,              /* --compat32-chroot */
    COMPAT32_PREFIX_OPTION,              /* --compat32-prefix */
    COMPAT32_LIBDIR_OPTION,              /* --compat32-libdir */
    UPDATE_OPTION,                       /* --update（未使用） */
    FORCE_SELINUX_OPTION,                /* --force-selinux */
    SELINUX_CHCON_TYPE_OPTION,           /* --selinux-chcon-type */
    NO_SIGWINCH_WORKAROUND_OPTION,       /* --no-sigwinch-workaround */
    X_MODULE_PATH_OPTION,                /* --x-module-path */
    DOCUMENTATION_PREFIX_OPTION,         /* --documentation-prefix */
    APPLICATION_PROFILE_PATH_OPTION,     /* --application-profile-path */
    X_LIBRARY_PATH_OPTION,              /* --x-library-path */
    NO_KERNEL_MODULES_OPTION,            /* --no-kernel-modules */
    NO_X_CHECK_OPTION,                   /* --no-x-check */
    NO_CC_VERSION_CHECK_OPTION,          /* --no-cc-version-check（已弃用） */
    NO_DISTRO_SCRIPTS_OPTION,            /* --no-distro-scripts */
    NO_OPENGL_FILES_OPTION,              /* --no-opengl-files */
    NO_WINE_FILES_OPTION,                /* --no-wine-files */
    KERNEL_MODULE_SOURCE_PREFIX_OPTION,  /* --kernel-module-source-prefix */
    KERNEL_MODULE_SOURCE_DIR_OPTION,     /* --kernel-module-source-dir */
    NO_KERNEL_MODULE_SOURCE_OPTION,      /* --no-kernel-module-source */
    DKMS_OPTION,                         /* --dkms / --no-dkms */
    MODULE_SIGNING_SECRET_KEY_OPTION,    /* --module-signing-secret-key */
    MODULE_SIGNING_PUBLIC_KEY_OPTION,    /* --module-signing-public-key */
    MODULE_SIGNING_SCRIPT_OPTION,        /* --module-signing-script */
    MODULE_SIGNING_KEY_PATH_OPTION,      /* --module-signing-key-path */
    MODULE_SIGNING_HASH_OPTION,          /* --module-signing-hash */
    MODULE_SIGNING_X509_HASH_OPTION,     /* --module-signing-x509-hash */
    INSTALL_VDPAU_WRAPPER_OPTION,        /* --install-vdpau-wrapper（已弃用） */
    NO_CHECK_FOR_ALTERNATE_INSTALLS_OPTION, /* --no-check-for-alternate-installs */
    NO_UVM_OPTION,                       /* --no-unified-memory */
    NO_DRM_OPTION,                       /* --no-drm */
    NO_PEERMEM_OPTION,                   /* --no-peermem */
    INSTALL_COMPAT32_LIBS_OPTION,        /* --install-compat32-libs / --no-install-compat32-libs */
    X_SYSCONFIG_PATH_OPTION,             /* --x-sysconfig-path */
    FORCE_LIBGLX_INDIRECT,               /* --force-libglx-indirect */
    NO_LIBGLX_INDIRECT,                  /* --no-libglx-indirect */
    INSTALL_LIBGLVND_OPTION,             /* --install-libglvnd / --no-install-libglvnd */
    GLVND_GLX_CLIENT_OPTION,             /* --glvnd-glx-client（已弃用） */
    GLVND_EGL_CONFIG_FILE_PATH_OPTION,   /* --glvnd-egl-config-path */
    GLVND_EGL_CLIENT_OPTION,             /* --glvnd-egl-client（已弃用） */
    EGL_EXTERNAL_PLATFORM_CONFIG_FILE_PATH_OPTION, /* --egl-external-platform-config-path */
    OVERRIDE_FILE_TYPE_DESTINATION_OPTION, /* --override-file-type-destination */
    SKIP_DEPMOD_OPTION,                  /* --skip-depmod */
    SYSTEMD_OPTION,                      /* --systemd / --no-systemd */
    SYSTEMD_UNIT_PREFIX_OPTION,          /* --systemd-unit-prefix */
    SYSTEMD_SLEEP_PREFIX_OPTION,         /* --systemd-sleep-prefix */
    SYSTEMD_SYSCONF_PREFIX_OPTION,       /* --systemd-sysconf-prefix */
    GBM_BACKEND_DIR_OPTION,              /* --gbm-backend-dir */
    GBM_BACKEND_DIR32_OPTION,            /* --gbm-backend-dir32 */
    ALLOW_INSTALLATION_WITH_RUNNING_DRIVER_OPTION, /* --allow-installation-with-running-driver */
    REBUILD_INITRAMFS_OPTION,            /* --rebuild-initramfs / --no-rebuild-initramfs */
    PRINT_RECOMMENDED_MODULE_TYPE_OPTION, /* --print-recommended-kernel-module-type */
};

/*
 * 命令行选项定义数组。
 * 每个元素是一个 NVGetoptOption 结构，包含：
 *   - name: 长选项名（如 "version"）
 *   - val: 短选项字符或枚举值
 *   - flags: 选项标志（NVGETOPT_STRING_ARGUMENT 等）
 *   - arg_name: 参数名（用于帮助文本中的占位符）
 *   - description: 帮助文本（NULL 表示隐藏选项）
 *
 * 数组以 { NULL, 0, 0, NULL, NULL } 结尾。
 */
